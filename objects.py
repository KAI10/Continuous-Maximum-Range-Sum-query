...
class Trajectory:
  ...
  def addLineToTrajectory(self,x1,y1,t1,speed=20):
    ...
    if (self.x_last==0 and self.y_last==0 and self.t_last==0)==False:
        t1=t1-self.offset
        distance = ((np.sqrt(np.power(self.x_last-x1, 2) + np.power(self.y_last-y1, 2)))/1000.0)/1.6093
        td = t1-self.t_last
        if td>0:
        ...
        t1=t1-self.offset
    if self.t_last!=t1:
        line = Line(self.parent_id, self.trajectory_id, self.count, self.x_last, self.y_last, self.t_last,x1,y1,t1,(calcspeed*1000*1.6093)/3600)
