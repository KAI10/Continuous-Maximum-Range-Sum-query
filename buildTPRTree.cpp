...
void showDataset()
{
    for(int i=0; i<objects.size(); i++){
        cout << objects[i].id << ":\n";
        cout << objects[i].inst[0].lat << ' ' << objects[i].inst[0].lon << ' ' << objects[i].inst[0].time << endl;
        cout << objects[i].inst[1].lat << ' ' << objects[i].inst[1].lon << ' ' << objects[i].inst[1].time << endl;
        cout << endl;
    }
}
...
int buildTree(Index* idx, int take)
{
    ...
    bool ok;
    for(int i=0; i<objects.size(); i++){
        if(objects[i].inst.size() <= take) continue; ///this object doesn't have 'take' NO. of line segments
...
int main(int argc, char** argv)
{
    ...
    Index* idx = createIndex(diskFileName);
    idx->flush();
